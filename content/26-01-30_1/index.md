...
### Phase 3. Contextual Retrieval (Anthropic)을 이용하자.
...
그래서 1) 불명확한 지칭 표현을 해결하고, 2) 시간의 흐름을 그대로 살리면서, 3) 문맥을 살릴 수 있는 방법을 찾은 것이

#### **2024년도에 Anthropic에서 발표한 "Contextual Retrieval"입니다.**

&nbsp;

이를 적용한 방법을 바로 얘기해보자면

> 1. **영상의 자막 전체**와 문맥을 추가할 자막 청크를 LLM에게 함께 주어 **전체 맥락을 파악하여 "해당 청크의 문맥"을 추가**해달라고 합니다.
> 2. 자막 청크 문서 내용에 **"문맥 + 기존 청크 내용"을 함께 넣어 검색 문서로 사용**합니다.

**청킹 방법은 자막의 시간대 한 구간을 묶어서** 하도록 했고,<br>**오버랩은 자막 문맥에 들어가지 않도록 metadata에 따로** 넣어두었습니다.(필요할 때 프롬프트 템플릿에 따로 꺼내어 쓰고자 합니다.)

&nbsp;

#### **문맥이 추가된 자막 한 청크를 보여드리겠습니다.**

```
문맥: 쯔양이 '굽네치킨'의 신메뉴인 맵단짠칩킨을 시식하는 장면입니다. 그녀는 치킨의 맛이 약간 매콤하면서도 단맛과 짭짤함이 조화를 이루어 중독성이 강하다고 평가합니다. 특히, 요블링소스에 치킨을 찍어 먹는 방법을 추천하며, 소스가 치킨의 맛을 한층 더 돋보이게 한다고 말합니다. 또한, 소스에 찍어 먹는 것이 더욱 맛있다고 강조하며, '굽네'의 소스 실력이 뛰어나다고 칭찬합니다.

[04:16 ~ 05:11] 막 맵진 않고 약간 매콤한데\n약간 단맛도 나고 짭짤한 맛도 나면서\n(멈출 수 없는 중독적인 맛-!!) 진짜 딱 중독성 있는 그런 맛이거든요?\n(뼈도 맛있옹ㅎ)\n진짜 감칠맛이 어마어마한 그런 맛?\n이번엔 소스에 찍어서!\n요블링소스에 푹 찍어서 먹어보겠습니다...!\n맛있다\n이게 약간 맵단짠이잖아요, 그래서\n요블링소스가 진짜 잘 어울려요\n감사합니다!\n(1인 2 뼈 통...ㅎ) 2개를ㅋㅋㅋ\n와, 근데 역시 '굽네'가\n소스를 진짜 잘해요!\n고추바사삭 먹을 때도 고블링, 마블링소스 찍어 먹잖아요!\n이번 소스도!
```

* 문맥은 **누가, 무엇을, 어떻게 하는지 문장형으로 서술하고, 음식명, 음식점명과 같은 고유명사는 그대로 사용**하여 생성하도록 했습니다.
  * 맵단짠'칩'킨의 고유명사를 그대로 사용함을 볼 수 있습니다.
* 문맥 아래의 자막에서 "진짜 감칠맛이 어마어마한 그런 맛"의 **대상이 불명확**했지만<br>**전체 자막 맥락에서 현재 맵단짠칩킨을 시식하고 있었음을 맥락적으로 확인**하여 서술하고 있음을 보여줍니다.

&nbsp;

#### **모델은 오픈소스 소형 모델을 이용했습니다.**

자막 텍스트 길이가 너무 길다 보니 비용적 문제를 신경쓰지 않을 수가 없었습니다.
...
> 2의 문제에서는
>
> **해당 자막 청크의 문맥만 출력하게끔 프롬프트 엔지니어링으로 유도를 해보아도<br>가끔 그 부분이 지켜지지 않은 경우가 있었습니다.**
>
...
> 정원분식과 떡볶당은 떡볶이와 분위기에서 어떻게 다른가요?
>
> 압구정동 식당의 스테이크 트러플 짜장면이 특별한 이유는 무엇인가요?
>
> 메뉴에 있는 매운 해산물 국수 요리가 무엇인가요?

이런 것들이 있었습니다.

> **simple: 0.4**, **reasoning: 0.2**, **multi_context: 0.4,**
>
> 의 비율로 평가셋을 생성했지만 전체 맥락적 요소를 파악하지 못한 부분들이 꽤 있었습니다.
...
특히 `Context Precision`, `Faithfulness`, `Context Recall` 3가지 지표가 크게 상승했음을 알 수 있습니다.
...
현재 RAG 개발에서는 입력과 관련한 문서를 '많이', '정확히' 가져오는 것이 중요해서<br>**`Context Precision`와 `Context Recall`가 주요한 지표였습니다.**

충분히 크게 개선이 되어<br>**문맥을 추가한 자막 문서를 이용하자고 결정**했습니다.
...
1. 자막은 **Sparse 벡터 검색도 추가**하여 **하이브리드 방식으로 검색하는 것이 좋을 것**이라 판단

   (특정 음식과 같은 고유명사가 들어간 사용자 입력이 있을 수 있어, 키워드 기반의 검색도 필요해보였습니다.)

2. **Cross Encoder 방식인 Reranker를 도입**하는 것이 좋을 것이라 판단

   (좀 더 빠른 ColBERT도 고민해보았지만 본 프로젝트에서는 관련 깊은 문서를 가져오는 것이 더 중요했습니다.)

하여 진행했습니다.

&nbsp;

여기서는 다양한 모델 테스트는 하지 못했습니다.<br>한국어 잘한다는 BGE 모델을 적극 활용했습니다.
...
을 비교했을 때 **Dense Vector(BGE-m3l) + Sparse Vector 검색(BGE-m3) + Reranker(BGE-reranker-v2-m3)** 방식이 더 성능이 좋았습니다.

&nbsp;

아까 개선했던 문맥 있는 자막 문서에서의 성능에서<br>앙상블 Retriever+리랭커 도입하면서 **모든 평가 지표에서 추가 개선**되었습니다 ‼️
...
>
> BGE-m3 임베딩 모델은 1024차원, OpenAI text-embedding-3-small 임베딩 모델은 1536차원입니다.<br>그런데도 BGE-m3 임베딩 모델의 성능이 더 좋았습니다.
>
> 임베딩 차원이 단순히 높은 것보다는, **한국어에 적합한 임베딩 모델을 선택하는 것**이 주요해보였습니다.<br>

&nbsp;

&nbsp;

해당 프로젝트에서 Agent가 해당 Retriver를 이용하도록 설계할 예정이라<br>RAG의 성능을 높이는 것이 필요했었습니다.

여기까지 프로젝트에서 RAG 개발 여정기였습니다. 끄읕.
